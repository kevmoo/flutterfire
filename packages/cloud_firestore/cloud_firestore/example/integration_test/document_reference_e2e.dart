  ...
  group('$DocumentReference', () {
    ...
    group('DocumentReference.snapshots()', () {
      test('returns a [Stream]', () async {
        DocumentReference<Map<String, dynamic>> document = await initializeTest(
          'document-snapshot',
        );
        Stream<DocumentSnapshot<Map<String, dynamic>>> stream = document
            .snapshots();
      ...
      test('listens to multiple documents', () async {
        DocumentReference<Map<String, dynamic>> doc1 = await initializeTest(
          'document-snapshot-1',
        );
        DocumentReference<Map<String, dynamic>> doc2 = await initializeTest(
          'document-snapshot-2',
        );
      ...
      test('listeners throws a [FirebaseException]', () async {
        DocumentReference<Map<String, dynamic>> document = firestore.doc(
    ...
    group('DocumentReference.set()', () {
      ...
      test('sets data with DocumentReference as map key', () async {
        DocumentReference<Map<String, dynamic>> document = await initializeTest(
          'document-set-ref-key',
        );
        DocumentReference<Map<String, dynamic>> refKey = FirebaseFirestore
            .instance
            .doc('foo/bar');
        await document.set({
          'myMap': {refKey: 42.0},
    ...
    group('DocumentReference.update()', () {
      ...
      test('updates nested data containing a dot using FieldPath', () async {
        ...
        DocumentSnapshot<Map<String, dynamic>> snapshot = await document.get();
        expect(snapshot.data(), equals({'foo.bar': 'baz'}));

        await document.update({
          FieldPath(const ['foo.bar']): 'toto',
        });
        DocumentSnapshot<Map<String, dynamic>> snapshot2 = await document.get();
        expect(snapshot2.data(), equals({'foo.bar': 'toto'}));
      });

      test('throws if document does not exist', () async {
        DocumentReference<Map<String, dynamic>> document = await initializeTest(
          'document-update-not-exists',
        );
        try {
    ...
    group('DocumentReference as field value', () {
      ...
      test('can query by DocumentReference value', () async {
        final collection = firestore.collection(
          'flutter-tests/doc-ref-query/items',
        );
        final targetDoc = firestore.doc('flutter-tests/target-doc');
